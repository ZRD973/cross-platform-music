<template>
    <div class="slider_wrapper">
        ...
        <div class="mod_slide">
            <ul class="mod_slide_list3">
                <li v-for="item in data" class="mod_slide_item">
                    <div class="mod_slide_item_box" @click="handleClickDisk(item)"> 
                        <div class="mod_slide_item_cove">
                            <el-image class="img" :src="`https://y.gtimg.cn/music/photo_new/T002R300x300M000${item.mid}.jpg`" fit="fill">
                        ...
                        <div class="mod_slide_item_title">
                            <span class="playlist__title_txt">{{item.name}}</span>
                        </div>
                        <div class="mod_slide_item_other">
                            <span v-for="sub in item.singers">{{sub.name}}</span>
...
export default {
    ...
    methods:{
        handleClickDisk(item){
            this.$emit("handleClickDisk",item)
        },
        chooseTab(item,index){
            this.number = 0;
            $('.mod_slide_list3').css({'left': '0%'})
            this.current = index;
            this.$emit("chooseTab",item)
        },
        slideSwitch(index){
            this.number = index;
            $('.mod_slide_list3').css({'left': '-' + index + '00%'})
        },
        prev_button(){
            this.number--;
            if(this.number==-1){
            ...
            $('.mod_slide_list3').css({'left': '-' + this.number + '00%'})
        },
        next_button(){
            this.number++;
