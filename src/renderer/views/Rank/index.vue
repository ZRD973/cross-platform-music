...
<script>
import { handleMusicTime } from "@/utils/helper";
import HomeLayout from "@/layouts/HomeLayout";
import ListTable from "@/components/listTable/ListTable.vue";
import ListCard from "@/components/listCard/ListCard.vue";
export default {
  components: { HomeLayout,ListTable, ListCard },
  name: "Ranking",
  data() {
    return {
      officialList: [],
  ...
  methods: {
    ...
    async getAllRankings() {
      let res = await this.$request.get("/toplist");
    ...
    async getMusicListDetail(id) {
      ...
      let result = await this.$request.get("/playlist/detail", { params: { id } } );
